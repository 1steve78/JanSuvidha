...
export default function ChatWidget() {
  ...
  return (
    <>
      ...
      <div className={`fixed bottom-6 right-6 z-50 transition-all duration-500 ${showWidget || isOpen ? "translate-y-0 opacity-100 pointer-events-auto" : "translate-y-10 opacity-0 pointer-events-none"}`}>
        ...
        <button
          onClick={toggleOpen}
          aria-label="Open JanSuvidha Chatbot"
      ...
      {/* Chat panel */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-[360px] max-h-[520px] flex flex-col bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden animate-in slide-in-from-bottom-4 fade-in duration-300">
          {/* Header */}
          <div className="bg-white border-b border-slate-100 px-4 py-3.5 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-slate-900 font-extrabold text-sm leading-none">JanSuvidha Saathi</p>
            ...
            {isLoading && (
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center shrink-0">
                  <span className="text-slate-500 text-[10px] font-black">JS</span>
                </div>
