...
const ACTION_LABELS: Record<string, { label: string; href: string; hi: string; bn: string }> = {
  match: {
      ...
      >
        ...
        >
          ...
          {!isUser && (
            <button
              ...
              className="absolute -bottom-2 -right-2 w-6 h-6 bg-indigo-100 hover:bg-indigo-200 text-indigo-600 rounded-full flex items-center justify-center shadow-sm transition-colors"
              title="Read aloud"
            >
        ...
        {actionConfig && actionLabel && (
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-bold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 px-3 py-1.5 rounded-xl transition-all hover:shadow-sm group"
          >
