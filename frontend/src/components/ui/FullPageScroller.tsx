...
const ENTRY: Array<gsap.TweenVars | null> = [
  ...
  { y: 90, opacity: 0, filter: "blur(10px)" },                  // 1 CivicJustice
  { scale: 0.88, opacity: 0, filter: "blur(8px)" },             // 2 PillarCards
  { x: -90, opacity: 0, filter: "blur(8px)" },                  // 3 HowItWorks
  { y: 80, opacity: 0 },                                         // 4 Stats+Trust
  { scale: 0.82, opacity: 0, filter: "blur(12px)" },            // 5 Map
  { x: 90, opacity: 0, filter: "blur(8px)" },                   // 6 Testimonial
  { y: 80, opacity: 0, filter: "blur(8px)" },                   // 7 FAQ
  { y: 50, opacity: 0 },                                         // 8 Footer
];
...
const LABELS = [
  "Home", "Civic Intelligence", "Our Features", "How It Works",
  "Impact", "Live Map", "Stories", "FAQ", "Contact",
];
...
export default function FullPageScroller({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    // Target ONLY footers that are NOT inside this scroller
