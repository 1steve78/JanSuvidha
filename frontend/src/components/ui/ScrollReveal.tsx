...
// ─── SnapSection ─────────────────────────────────────────────────────────────
/**
 * A full-screen scroll-snap section.
 * - Takes up exactly 100dvh (minus the sticky header which is ~96px).
 * - Snaps the section to the viewport when scrolling.
 * - Content-heavy sections: pass overflowY="auto" so inner content is scrollable.
 * - animates in with the chosen preset when it enters the viewport.
 */
interface SnapSectionProps {
  children: ReactNode;
  className?: string;
  /** bg colour / gradient class for this section */
  bg?: string;
  preset?: Preset;
  delay?: number;
  /** Allow inner overflow-y scroll when content is taller than viewport */
  overflowY?: "auto" | "hidden" | "visible";
  /** Custom id for anchor linking */
  id?: string;
  /** Remove the default padding on the inner content wrapper */
  noPad?: boolean;
}
...
export function SnapSection({
  ...
  return (
    <section
      id={id}
      className={`snap-section relative flex flex-col ${bg} ${className}`}
      style={{ overflowY }}
    >
      <motion.div
        variants={variants}
