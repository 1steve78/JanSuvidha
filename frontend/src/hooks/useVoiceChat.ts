...
import { useChatStore, ChatLanguage } from "@/lib/store";
...
function getBestVoice(synth: SpeechSynthesis, lang: ChatLanguage): SpeechSynthesisVoice | null {
  ...
  const primary = code.split("-")[0]; // "bn", "hi", "en"

  // 1. Exact locale match (e.g. "bn-BD" or "bn-IN")
...
export function useVoiceChat(onTranscript?: (text: string) => void) {
  ...
  const speak = useCallback(
    (text: string, lang: ChatLanguage = language) => {
      ...
      utterance.onerror = (e) => {
        // "not-allowed" = page not focused; "synthesis-failed" = no voice. Both are silent fails.
        setSpeaking(false);
      };
